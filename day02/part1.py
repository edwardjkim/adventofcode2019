...
def string_to_list(opcode_string: str) -> List[int]:
    return [int(x) for x in opcode_string.split(',')]


def list_to_string(list_of_ints: List[int]) -> str:
    return ','.join(str(x) for x in list_of_ints)


def single_process(sequence: List[int], idx: int) -> None:
    if idx + 3 >= len(sequence):
        raise IndexError
    value1 = sequence[sequence[idx + 1]]
    value2 = sequence[sequence[idx + 2]]
    if sequence[idx] == 1:
        sequence[sequence[idx + 3]] = value1 + value2
    elif sequence[idx] == 2:
        sequence[sequence[idx + 3]] = value1 * value2
    else:
...
def process(opcode_string: str) -> str:
...
def restore_program(opcode_string: str) -> str:
    opcode_int_sequence = string_to_list(opcode_string)
    opcode_int_sequence[1] = 12
    opcode_int_sequence[2] = 2
    for i in range(0, len(opcode_int_sequence), 4):
        if opcode_int_sequence[i] == 99:
            break
        single_process(opcode_int_sequence, i)
