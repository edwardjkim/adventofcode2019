...
"""
from part1 import find_fuel_required, parse_input


def find_fuel_required_recursive(mass: int) -> int:

    fuel_required = find_fuel_required(mass)
    if fuel_required <= 0:
        return 0
    return fuel_required + find_fuel_required_recursive(fuel_required)


assert find_fuel_required_recursive(14) == 2
assert find_fuel_required_recursive(1969) == 966
assert find_fuel_required_recursive(100756) == 50346


def main():
    print(sum(find_fuel_required_recursive(x) for x in parse_input("input")))
