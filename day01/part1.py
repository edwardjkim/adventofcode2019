...
def find_fuel_required(mass: int) -> int:
    """
    Finds the fuel required to launch a given module based on its mass.
    """
    return int(mass / 3) - 2


assert find_fuel_required(12) == 2
assert find_fuel_required(14) == 2
assert find_fuel_required(1969) == 654
assert find_fuel_required(100756) == 33583


def parse_input(input_file):
    with open(input_file) as f:
        lines = f.read().split()
    return [int(line.strip()) for line in lines if line]


def main():
    print(sum(find_fuel_required(x) for x in parse_input("input")))
